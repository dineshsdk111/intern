...
import {
  chicagoPneumaticCategories,
  chicagoPneumaticBrandInfo,
  BrandProduct,
} from "@/data/chicago-pneumatic";

export default function ChicagoPneumaticBrandPage() {
  const [activeCategory, setActiveCategory] = useState(
    chicagoPneumaticCategories[0].slug
  );
  const [selectedTool, setSelectedTool] = useState<BrandProduct | null>(null);

  const currentCategory = chicagoPneumaticCategories.find(
  ...
  return (
    <>
      ...
      <section className="relative py-24 bg-navy overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://tools.cp.com/media/wysiwyg/hero-bg.jpg')] bg-cover opacity-10" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-sm text-white/60 mb-4">
            ...
            <span className="text-white">{chicagoPneumaticBrandInfo.name}</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10">
            <div className="flex-1">
              ...
              <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4">
                {chicagoPneumaticBrandInfo.name}
              </h1>
              <p className="text-lg text-white/70 mb-6 max-w-xl">
                {chicagoPneumaticBrandInfo.description}
              </p>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                {chicagoPneumaticBrandInfo.stats.map((stat) => (
                  <div
                    key={stat.label}
              ...
              <div className="flex flex-wrap gap-3">
                <a
                  href={chicagoPneumaticBrandInfo.website}
                  target="_blank"
            ...
            <div className="w-full lg:w-80 flex-shrink-0">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                <img
                  src={chicagoPneumaticBrandInfo.logo}
                  alt="Chicago Pneumatic Logo"
                  className="w-full h-32 object-contain mb-4"
                />
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-white/80">
                    <Factory className="w-4 h-4 text-accent" />
                    <span>Founded: {chicagoPneumaticBrandInfo.founded}</span>
                  </div>
                  <div className="flex items-center gap-2 text-white/80">
                    <Wrench className="w-4 h-4 text-accent" />
                    <span>HQ: {chicagoPneumaticBrandInfo.headquarters}</span>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {chicagoPneumaticBrandInfo.specialties.map((s) => (
                    <span
                      key={s}
      ...
      <section className="sticky top-16 z-30 bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex overflow-x-auto no-scrollbar gap-1 -mb-px">
            {chicagoPneumaticCategories.map((cat) => (
              <button
                key={cat.slug}
      ...
      <section className="py-16 bg-navy">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white mb-4">
            Need Chicago Pneumatic Tools for Your Project?
          </h2>
          <p className="text-white/70 max-w-xl mx-auto mb-8">
            Contact us for competitive pricing, bulk orders and expert
            recommendations on the right CP tools for your application.
          </p>
