...
import {
  makitaToolCategories,
  makitaBrandInfo,
  MakitaTool,
} from "@/data/makita-tools";

export default function MakitaBrandPage() {
  const [activeCategory, setActiveCategory] = useState(
    makitaToolCategories[0].slug
  );
  const [selectedTool, setSelectedTool] = useState<MakitaTool | null>(null);

  const currentCategory = makitaToolCategories.find(
  ...
  return (
    <>
      ...
      <section className="relative py-24 bg-navy overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://makita.in/wp-content/themes/Makita/img/banner_particle.png')] bg-cover opacity-10" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-sm text-white/60 mb-4">
            ...
            <span className="text-white">{makitaBrandInfo.name}</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10">
            <div className="flex-1">
              ...
              <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4">
                {makitaBrandInfo.name}
              </h1>
              <p className="text-lg text-white/70 mb-6 max-w-xl">
                {makitaBrandInfo.description}
              </p>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                {makitaBrandInfo.stats.map((stat) => (
                  <div
                    key={stat.label}
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-white/80">
                    ...
                    <span>Founded: {makitaBrandInfo.founded}</span>
                  </div>
                  <div className="flex items-center gap-2 text-white/80">
                    <Wrench className="w-4 h-4 text-accent" />
                    <span>HQ: {makitaBrandInfo.headquarters}</span>
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  {makitaBrandInfo.specialties.map((s) => (
                    <span
                      key={s}
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden bg-surface">
                      ...
                      {tool.voltage && (
                        <div className="absolute top-3 left-3">
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-white bg-navy/80 backdrop-blur-sm px-2 py-1 rounded-full">
            ...
            <div className="relative h-56 overflow-hidden">
              ...
              <div className="absolute bottom-5 left-6 right-6">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {selectedTool.voltage && (
                    <span className="text-[10px] font-bold text-white uppercase tracking-wider bg-accent/80 px-2.5 py-1 rounded-full">
                      {selectedTool.voltage}
                    </span>
                  )}
                </div>
      ...
      <section className="py-16 bg-navy">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white mb-4">
            Need Makita Tools for Your Project?
          </h2>
          <p className="text-white/70 max-w-xl mx-auto mb-8">
            Contact us for competitive pricing, bulk orders and expert
            recommendations on the right Makita tools for your application.
          </p>
